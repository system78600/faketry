...
async def log_pro_broadcast_usage(user_id):
    ...
    if user_data:
        ...
        if current_time - last_broadcast >= timedelta(hours=5):
            ...
            time_until_next_broadcast = (
                last_broadcast + timedelta(hours=5)
            ) - current_time
            return False, time_until_next_broadcast
    else:
        await protimes.insert_one(
            {"user_id": user_id, "broadcast_count": 1, "last_broadcast": current_time}
        )
        return True
...
async def broadcast(client: Client, message: Message):
    ...
    if IS_BROADCASTING:
        return await message.reply_text(
            "A broadcast is already in progress. Please wait until it finishes."
        )
    ...
    if not can_broadcast:
        return await message.reply_text(
            f"{user.mention} can only use /gcast 2 times in a 3-hour period. Your daily broadcasts are finished. Try again tomorrow."
        )
    if "-wfchat" in message.text or "-wfuser" in message.text:
        ...
        reply_markup = (
            ...
            if hasattr(message.reply_to_message, "reply_markup")
            else None
        )
                ...
                try:
                    if content_type == "photo":
                        ...
                        await app.send_message(
                            chat_id=i, text=text_content, reply_markup=reply_markup
                        )
                    sent_chats += 1
...
@app.on_message(filters.command("addteam") & filters.user(OWNER_ID))
async def addpro_handler(client: Client, message: Message):
    if not message.reply_to_message and len(message.command) < 2:
        return await message.reply_text(
            "Provide a user ID or reply to a message to add a user as pro."
        )
    ...
    if len(message.command) >= 3:
        try:
            ...
            return await message.reply_text(
                "Invalid duration. Please provide the number of days."
            )
    else:
        duration = 1
    is_pro_user = await is_pro(user.id)
                ...
                [
                    [
                        InlineKeyboardButton(
                            "Yes", callback_data=f"update_pro:{user.id}:{duration}"
                        )
                    ],
    ...
    return await message.reply_text(
        f"{user.mention} has been added to pro for {duration} days."
    )
...
@app.on_callback_query(
    filters.regex(r"update_pro:(\d+):(\d+)") & filters.user(OWNER_ID)
)
async def update_pro_callback(client: Client, callback_query):
    ...
    await pros.update_one({"user_id": user_id}, {"$set": {"expires": new_expiration}})
    await callback_query.message.edit_text(
        f"Pro status for user ID {user_id} has been updated. New expiration: {new_expiration.strftime('%Y-%m-%d %H:%M:%S')}."
    )


@app.on_message(filters.command("rmteam") & filters.user(OWNER_ID))
async def rmpro_handler(client: Client, message: Message):
    if not message.reply_to_message and len(message.command) < 2:
        return await message.reply_text(
            "Provide a user ID or reply to a message to remove the user from pro."
        )
    user = await extract_user(message)
    if not user:
        return await message.reply_text("Could not find the user.")
...
@app.on_message(filters.command("teamlist"))
